...
| **UNS**    | In-app notifications integration | Sibling internal service           | SigV4, cross-account role | UNS team directly; they can fix forward on their side           |
...
The practical difference: for **UDP** and **UNS** you can usually reach the owning team and they can deploy a fix; for **DVLA**, a third party outside the programme, you are largely limited to mitigating on the FLEX side and relaying the provider's status.
...
4. **Check the upstream's status.** For **DVLA**, check any published provider status page. For **UNS** and **UDP**, ask the owning team directly. An acknowledged upstream incident confirms the diagnosis immediately.
