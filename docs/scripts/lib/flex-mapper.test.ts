...
describe("mapSchemaObject", () => {
  ...
  it("treats keys without ! as optional", () => {
    const result = mapSchemaObject({
      "name": "string!",
      "parent": "Authority",
    });
...
describe("mapContract", () => {
  ...
  it("expands array shorthand in response and body", () => {
    const result = mapContract({
      openapi: "3.1.0",
      info: { title: "Test", version: "1.0.0" },
      paths: {
    ...
    const getSchema = (
      ((get.responses as Record<string, Record<string, unknown>>)["200"]
