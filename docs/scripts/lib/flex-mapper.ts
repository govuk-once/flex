...
function mapRouteOperation(op: JsonObject): JsonObject {
  ...
  if (typeof result.response === "string") {
    const schema = mapSchemaValue(result.response);
  ...
  if (typeof result.body === "string") {
    const schema = mapSchemaValue(result.body);
