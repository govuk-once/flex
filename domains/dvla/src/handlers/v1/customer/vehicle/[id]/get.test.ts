...
describe("GET /v1/customer/vehicle/:id", () => {
  const endpoint = "/customer/vehicle/:id";
  const vehicleId = "12345";

  it("returns 200 with the customer vehicle details", async ({ http, sdk }) => {
    const mockVehicleResponse = {
      customerVehicleDetails: {
        ...
        make: "FORD",
        motStatus: "Valid",
        dateOfFirstRegistration: "2020-01-01",
        fuelType: "PETROL",
        colour: "BLACK",
      },
    };
    ...
    expect(JSON.parse(result.body)).toStrictEqual(mockVehicleResponse);
  });
  ...
  it.for([
    ...
    "returns $expected when the DVLA get customer vehicle integration $reason",
    async ({ upstream, expected }, { http, sdk }) => {
      http
        .domain("udp")
        .get("/identity/dvla", { headers: { "User-Id": userId } })
